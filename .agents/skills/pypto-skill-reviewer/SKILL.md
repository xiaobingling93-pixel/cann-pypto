...
基于 9 个维度下的 47 条规则，对一个 skill 目录进行评审，并产出带有可执行修复建议的评分报告。
...
|------|---------|-------------|
| [references/rules.json](references/rules.json) | 47 条规则、维度、权重和严重级别的唯一事实来源 | 在第 1 阶段和第 2 阶段开始时读取 |
...
3. **规则覆盖率** —— 静态计数 + 语义计数 = 总评估数，含按状态拆分（PASS/FAIL/SKIP），覆盖率百分比 = evaluated / 47 × 100%
