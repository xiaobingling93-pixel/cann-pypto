...
enum class MatmulErrorCode : uint32_t {
    ...
    // FC3xxx: 参数错误
    ERR_PARAM_INVALID       = 3000U,  // 参数无效（shape、dtype、format等）
    ERR_PARAM_MISMATCH      = 3001U,  // 参数不匹配（维度、类型、K轴等）
    ERR_PARAM_UNSUPPORTED   = 3002U,  // 不支持的参数
    
    // FC4xxx: 配置错误
    ERR_CONFIG_TILE         = 4000U,  // Tile 配置错误
    ERR_CONFIG_ALIGNMENT    = 4001U,  // 对齐错误（16B/32B/64元素）
    ERR_CONFIG_UNSUPPORTED  = 4002U,  // 不支持的配置组合
    
    // FC5xxx: 运行时错误
    ERR_RUNTIME_NULLPTR     = 5000U,  // 空指针错误
    ERR_RUNTIME_STATE       = 5001U,  // 内部状态错误
    ERR_RUNTIME_LOGIC       = 5002U,  // 逻辑不变量错误
};
