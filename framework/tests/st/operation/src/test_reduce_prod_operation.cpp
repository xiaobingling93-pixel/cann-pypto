...
void ProdOperationExeFunc(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
...
void Prod3DOperationExeFunc(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                 const OpFuncArgs* opArgs) {
    auto args = static_cast<const ProdOpFuncArgs *>(opArgs);
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
...
void Prod4DOperationExeFunc(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs,
                                 const OpFuncArgs* opArgs) {
    auto args = static_cast<const ProdOpFuncArgs *>(opArgs);
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
...
INSTANTIATE_TEST_SUITE_P(TestProd, ProdOperationTest, ::testing::ValuesIn(
    GetOpMetaData<ProdOpMetadata>({ProdOperationExeFunc, Prod3DOperationExeFunc, Prod4DOperationExeFunc}, "Prod")));
