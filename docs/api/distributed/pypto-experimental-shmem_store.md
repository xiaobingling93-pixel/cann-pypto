...
- 示例 1：先创建一个 shared memory tensor，其形状为 [1] + 输入数据的形状。将输入数据赋值到 pe = 2 的 shared memory tensor 的指定区域，并与该视图原本的数据进行累加操作。注意，shared memory tensor 的 dtype 和 输入数据的 dtype 必须一致。
